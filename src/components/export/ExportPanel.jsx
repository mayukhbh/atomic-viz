...
export function ExportPanel({onClose}){
 ...
 <div className="space-y-3"><button disabled={busy} onClick={()=>run(()=>captureScreenshot(sceneRef?.current?.canvas))} className="w-full p-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl flex items-center gap-3"><Camera className="text-cyan-400" size={20}/>Screenshot (PNG)</button><button disabled={busy} onClick={()=>run(()=>exportGLTF(sceneRef?.current?.scene))} className="w-full p-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl flex items-center gap-3"><Box className="text-purple-400" size={20}/>3D Model (GLTF)</button><p className="text-xs text-white/60">GLTF saves a static mesh snapshot. Labels, electron trails, stars, and lighting effects are available in PNG only.</p><p role="status" className="text-sm text-cyan-200">{message}</p></div></motion.div>;
}
